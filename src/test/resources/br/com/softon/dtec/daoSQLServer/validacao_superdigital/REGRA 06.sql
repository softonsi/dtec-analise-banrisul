		     ...
		     WHERE CD_IDENTF_PERFIL = 3
			   ...
			   AND CD_VARIAVEL_SEGUNDA  = T.CD_TP_IDENTF_CLIE
	  		   AND cd_ano_mes = CASE WHEN MONTH(T.DT_TRANS) < 10
	                            ...
	                            END 	 
	       ) >=  (select vl_param from tb_regra_parametro where nm_campo_param = 'pm_QContasDistintas' and cd_regra = 6 and cd_versao_sistema = 3)
	  OR 
		     ...
		     WHERE CD_IDENTF_PERFIL = 3
			   ...
			   AND CD_VARIAVEL_SEGUNDA  = T.CD_TP_IDENTF_CLIE
	  		   AND cd_ano_mes = CASE WHEN MONTH(T.DT_TRANS) < 10
	                                 ...
	                                 ELSE CAST(YEAR(T.DT_TRANS) AS VARCHAR) +       CAST(MONTH(T.DT_TRANS) AS VARCHAR)
	                            END 	
	       ) >= (select vl_param from tb_regra_parametro where nm_campo_param = 'pm_VlCargaMensal' and cd_regra = 6 and cd_versao_sistema = 3)
       )
) s; 
