...
where (i.cd_lista_auxiliar = l.cd_lista_auxiliar and (dateadd(day,i.nu_prazo_validd_dia,i.dt_inclusao) >= getdate() or i.nu_prazo_validd_dia is null)) AND 
      T.CD_LOTE = 2014091801 AND T.CD_SUBLOTE = 1 AND I.DS_CONTEUDO = T.NM_CLIE AND l.cd_lista_auxiliar = 203 AND
...
select count(1) as total from tb_detlh_apontamento where cd_lote = 2014091801 and cd_regra = 305
