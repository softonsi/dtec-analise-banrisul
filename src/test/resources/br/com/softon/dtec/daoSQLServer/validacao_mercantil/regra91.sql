...
WHERE CD_TP_PESSOA = 'F'
  ...
  AND ((VL_RENDA_FAT IS NOT NULL AND VL_PCELA_CONTRATO > (VL_RENDA_FAT * (SELECT VL_PARAM/100 FROM TB_REGRA_PARAMETRO WHERE CD_REGRA = 91 AND CD_VERSAO_SISTEMA = 3 AND NM_CAMPO_PARAM = 'pm_percRenda')))
...
WHERE T.CD_LOTE = 2014091801
  ...
  AND T.CD_TP_OPER = 31
  AND T.VL_PCELA_CONTRATO IS NOT NULL
  AND ((T.VL_RENDA_FAT IS NOT NULL AND T.VL_PCELA_CONTRATO > (VL_RENDA_FAT * (SELECT VL_PARAM/100 FROM TB_REGRA_PARAMETRO WHERE CD_REGRA = 91 AND CD_VERSAO_SISTEMA = 3 AND NM_CAMPO_PARAM = 'pm_percFatmt')))
