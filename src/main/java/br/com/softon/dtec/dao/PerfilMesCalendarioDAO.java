...
import static br.com.softon.dtec.utils.CarregarQuery.Origem.PERFIL_MES_CALENDARIO;
...
import br.com.softon.dtec.entity.ConjuntoPerfisMesCalendario;
import br.com.softon.dtec.entity.PerfilMesCalendario;
import br.com.softon.dtec.entity.Sublote;
import br.com.softon.dtec.entity.VariavelPerfil;

public class PerfilMesCalendarioDAO {

    private final static Logger log = LoggerFactory.getLogger(PerfilMesCalendarioDAO.class);

    private static final String QUERY = carregarQuery(PERFIL_MES_CALENDARIO, "query.sql");

    private static final String QUERY_VARIAVEL_PERFIL = carregarQuery(VARIAVEL_PERFIL, "query.sql");

    public static ConjuntoPerfisMesCalendario getConjuntoPerfisMesCalendario(Sublote sublote) throws Exception {
        log.debug("Construindo query...");
        Connection con = null;
        ResultSet rs = null;
        Set<PerfilMesCalendario> result = new HashSet<PerfilMesCalendario>();
        ...
        try {
            ...
            while (rs.next()) {
                final PerfilMesCalendario perfil = new PerfilMesCalendario(
                		rs.getString("CD_ANO_MES") != null ? rs.getInt("CD_ANO_MES") : null,
            ...
            log.debug(result.size() + " Perfis Mes Calendario carregados.");
            return new ConjuntoPerfisMesCalendario(result);
        } catch (SQLException e) {
            log.error("Houve um erro ao buscar dados de Perfis Mes Calendario...");
            throw e;
        } catch (Exception e) {
            log.error("Houve um erro ao buscar dados de Perfis Mes Calendario...");
            throw e;
	...
	private static Collection<VariavelPerfil> buscarVariaveisPerfilInformacao() throws Exception {
        ...
        try {
            ...
            ps.setByte(1, (byte) 2); // 2 - Tipo Perfil Mês Calendário
            rs = ps.executeQuery();
            log.debug("Query executada, buscando resultados da query...");
            while (rs.next()) {
            ...
            log.debug(result.size() + " Variáveis de Perfil Mês Calendário carregados.");
            return result;
        } catch (SQLException e) {
            log.error("Houve um erro ao buscar dados de Variáveis de Perfil Mês Calendário...");
            throw e;
        } catch (Exception e) {
            log.error("Houve um erro ao buscar dados de Variáveis de Perfil Mês Calendário...");
            throw e;
